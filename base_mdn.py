...
class LitEncoderDecoder(pl.LightningModule):
    def __init__(self, encoder: nn.Module, decoder: nn.Module,
        ...
        self.annealing_epochs = int(self.max_epochs * 0.9)
    ...
    def encode_decode(self, data, batch_idx, tf_prob=0.0):
        ...
        for di in range(target_length):
            dec_in = (decoder_input, decoder_hidden, encoder_output,
                       data.edge_index[-1], data.edge_features[-1], past_state, dec_static_features)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.parameters(), lr=self.learning_rate, betas=(0.9, 0.999))
        scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=1, end_factor=0.001,
